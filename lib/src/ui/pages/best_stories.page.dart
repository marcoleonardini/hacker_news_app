...
class BestStoriesPage extends StatefulWidget {
  @override
  _NewStoriesPageState createState() => _NewStoriesPageState();
}

class _NewStoriesPageState extends State<BestStoriesPage>
      ...
      child: RefreshIndicator(
        ...
        child: Consumer<StoryProvider>(
          builder: (context, storyProvider, widget) {
            print(storyProvider.bestStoriesIds.length);
            if (storyProvider.bestStoriesIds.length == 0) return Container();
            return ListView.builder(
              itemCount: storyProvider.bestStoriesIds.length,
              itemBuilder: (context, index) {
                return FutureBuilder<Story>(
                  future: StoryService()
                      .getStory(storyProvider.bestStoriesIds[index]),
